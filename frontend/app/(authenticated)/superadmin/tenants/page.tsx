import CustomButton from "@/components/Button/Button";
import TenantStatsCard from "@/components/TenantStatsCard/TenantStatsCard";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { DataTable } from "@/components/ui/data-table";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuLabel,
  DropdownMenuSeparator,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import {
  AlertTriangle,
  Box,
  Building,
  DollarSign,
  Ellipsis,
  LucideCalendarRange,
  Plus,
  PlusIcon,
  UsersRoundIcon,
} from "lucide-react";
import React from "react";
import { tenantColumns } from "../_component/tenantColumn";
import { Tenant } from "@/types/types";

const tenantsData: Tenant[] = [
  {
    id: "TNT001",
    name: "SuperMarket Chain A",
    email: "admin@supermarketa.com",
    plan: "enterprise",
    branches: 15,
    employees: 450,
    status: "active",
  },
  {
    id: "TNT002",
    name: "Fresh Foods Network",
    email: "contact@freshfoods.com",
    plan: "premium",
    branches: 8,
    employees: 220,
    status: "active",
  },
  {
    id: "TNT003",
    name: "Metro Grocery Co.",
    email: "info@metrogrocery.com",
    plan: "premium",
    branches: 12,
    employees: 380,
    status: "active",
  },
  {
    id: "TNT004",
    name: "Corner Store Plus",
    email: "admin@cornerstore.com",
    plan: "basic",
    branches: 3,
    employees: 45,
    status: "active",
  },
  {
    id: "TNT005",
    name: "City Market Group",
    email: "support@citymarket.com",
    plan: "enterprise",
    branches: 20,
    employees: 650,
    status: "suspended",
  },
];

const Page = () => {
  return (
    <div className="container-base">
      <header className="flex flex-col gap-6">
        {/* Welcome Message and the Title With a Button */}
        <div className="flex items-center justify-between">
          <section>
            <h1 className="text-2xl font-bold m">Tenants Management</h1>
            <p className="text-muted-foreground">
              Manage all tenant accounts, subscriptions, and configurations.
            </p>
          </section>
        </div>

        {/* Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 w-full">
          <TenantStatsCard title="Total Items" value="5" icon={<Box />} />
          <TenantStatsCard
            title="Total Tenants"
            value="1"
            icon={<AlertTriangle />}
          />
          <TenantStatsCard
            title="Active Tenants"
            value="3"
            icon={<LucideCalendarRange />}
          />
          <TenantStatsCard
            title="Total Revenue"
            value="$1.9M"
            icon={<DollarSign />}
          />
          <TenantStatsCard
            title="Total Employees"
            value="1,745"
            icon={<UsersRoundIcon />}
          />
        </div>
      </header>

      <main className="flex flex-col gap-6">
        {/* Branches Management */}
        <section>
          <Card className="w-full">
            <CardHeader className="flex flex-col">
              <CardTitle className="text-2xl font-bold">All Tenants</CardTitle>
              <p className="text-xs text-muted-foreground flex items-center">
                Manage tenant accounts, subscriptions, and access permissions
              </p>
            </CardHeader>
            <CardContent>
              {/* Table */}
              <DataTable
                columns={tenantColumns}
                data={tenantsData}
                filterBy="name"
              />
            </CardContent>
          </Card>
        </section>
      </main>
    </div>
  );
};

export default Page;
